...
#'   strictly between 0 and 1. If `af` has names in `CHR:POS:REF:ALT` format,
#'   these names are used as variant IDs in the returned matrix. Otherwise,
#'   variant IDs are generated as `chr0:1:A:C`, `chr0:2:A:C`, and so on.
...
#'   Row names are taken from `names(af)` when all names are in `CHR:POS:REF:ALT`
#'   format; otherwise, they are generated as `chr0:<index>:A:C`. Column names
#'   are taken from the individual IDs in `adata$Lmat`.
#'
#' @details
...
#'
#' The `CHR:POS:REF:ALT` check is intentionally simple and expects variant names
#' such as `chr1:12345:A:C` or `1:12345:A:C`. Multi-allelic ALT strings separated
#' by commas are also accepted.
#'
#' @examples
#' \dontrun{
